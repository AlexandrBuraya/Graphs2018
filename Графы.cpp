// Графы.cpp: определяет точку входа для консольного приложения.
//
...
class RepresType { // вспомогательный класс для представления графа (скелет представления графа)
protected:
	int vertex_num = 0;
	bool is_weighted;
	bool is_oriented;
...
private:
	int size = 0; //кол-во изолированных элементов 
	vector <int> rank;
	vector <int> parent;
public:
	int find(int x) {
...
public:
	...
	void readGraph(string fileName) {//прочтение данных из текстового файла
		ifstream file(fileName);
		char repr; //сигнал в файде, в какрм типе представлен граф в файле
		file >> repr;
		switch (repr)
		{
		case 'C':   repres = new AdjMatrixGraph(); break; //представление матрицей смежности
		case 'L':   repres = new AdjListGraph(); break; //представление списком смежных вершин
		case 'E':   repres = new ListOfEdgesGraph(); break; //представление списком ребер
		}
		repres->readGraph(file);
		file.close();
	}

	void addEdge(int from, int to, int weight = 1) {//метод добавляющий ребро
		repres->addEdge(from, to, weight);
	}

	void removeEdge(int from, int to) {//метод удаляющий ребро
		repres->removeEdge(from, to);
	}

	int changeEdge(int from, int to, int newWeight) {//метод возвращает срарое значение веса ребра
		return repres->changeEdge(from, to, newWeight);
	}

	void transformToAdjMatrix() {//преобразование в представление матрицей смежности
		repres = new AdjMatrixGraph(repres->transformToAdjMatrix(), repres->GetInfo());
	}

	void transformToAdjList() {//преобразование в представление списком смежных вершин
		repres = new AdjListGraph(repres->transformToAdjList(), repres->GetInfo());
	}

	void transformToListOfEdges() {//преобразование в представление списком ребер
		repres = new ListOfEdgesGraph(repres->transformToListOfEdges(), repres->GetInfo());
	}

	void writeGraph(string fileName) {//запись графа в файл вывода
		repres->writeGraph(fileName);
	}

	Graph getSpaingTreePrima() {//Алгоритм Прима
		this->transformToAdjList(); //преобразование в список смежных вершин
	...
	Graph getSpaingTreeKruscal() {//Алгоритм Краскала
		this->transformToListOfEdges(); //преобразование в список ребер
	...
	Graph getSpaingTreeBoruvka() {//Алгоритм Борувки
		this->transformToListOfEdges(); //преобразование в список ребер
		vector <tuple<int, int, int>> minimalSpanningTree = reinterpret_cast<ListOfEdgesGraph*>(repres)->getSpaingTreeKruscal();
		Graph* spaingTree = new Graph();
		spaingTree->repres = new ListOfEdgesGraph(minimalSpanningTree, repres->GetInfo());
	...
	int checkBipart(vector<char> &marks) {//проверка на двудольность
		this->transformToAdjList();
		return reinterpret_cast<AdjListGraph*>(repres)->checkBipart();
	}

	vector<pair<int, int> > getMaximumMatchingBipart() {//Алгорит Куна
		this->transformToAdjList();//преобразование в список смежных вершин
